...
CASE WHEN ci.cst_gndr != 'N/A' THEN ci.cst_gndr -- DATA INTEGRATION ON GENDER
	 ELSE COALESCE(ca.gen, 'N/A')
END AS gender,ca.bdate AS birthdate,
ci.cst_create_date AS create_date
FROM silver.crm_cust_info ci 
LEFT JOIN silver.erp_cust_az12 ca ON TRIM(ci.cst_key) = TRIM(ca.cid)
LEFT JOIN silver.erp_loc_a101 la ON TRIM(ci.cst_key) = TRIM(la.cid);
